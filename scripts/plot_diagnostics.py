...
def plot_density_profile(path):
    ...
    N_DM = len(bodies)
    r_vals = np.array([(bodies[i].position.length()).in_(units.parsec).number for i in range(N_DM)])
    
    N_bins = 50
    r_bins = np.geomspace(1e-11, 1e-5, N_bins+1)
    
    r_counts, r_bins = np.histogram(r_vals, r_bins)
    r_counts = r_counts*bodies[0].mass.in_(units.MSun).number #Multiply by mass per particle
