...
@click.option("--softening_length", default=1e-10, help="softening length (pc)")
@click.option("--accuracy_parameter", default=0.1)
@click.option("--manage_encounters", default=1)
@click.option("--end_time", default=100.0)
...
def run(
    ...
    converter = nbody_system.nbody_to_si(1.0 | units.MSun, 1.0 | units.kms)
